...
`@apty/agent-core` wraps `@openai/agents` and adds a few opinionated layers that are
...
```bash
npm install @apty/agent-core
# or
pnpm add @apty/agent-core
```

### Optional peer dependencies (model providers)

`@apty/agent-core` integrates well with the AI SDK ecosystem. Install the provider(s) you plan to use:
...
```ts
import { google } from "@ai-sdk/google";
import { AIPex, aisdk, calculatorTool, httpFetchTool } from "@apty/agent-core";
...
```ts
import { google } from "@ai-sdk/google";
import { AIPex, aisdk, ContextManager, type Context } from "@apty/agent-core";
...
```ts
import { google } from "@ai-sdk/google";
import { AIPex, aisdk, type AgentPlugin } from "@apty/agent-core";
...
```ts
import { z } from "zod";
import { ToolRegistry } from "@apty/agent-core";
...
```bash
pnpm --filter @apty/agent-core build
pnpm --filter @apty/agent-core typecheck
pnpm --filter @apty/agent-core test
```

Run the included examples:

```bash
pnpm --filter @apty/agent-core example:basic
pnpm --filter @apty/agent-core example:fork
pnpm --filter @apty/agent-core example:metrics
```
