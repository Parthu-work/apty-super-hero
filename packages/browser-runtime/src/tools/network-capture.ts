 ...
 * These three tools implement the alternative flow: start a capture, let
 * the user reproduce the issue across as many turns as it takes, then stop
 * and get everything that happened in between — see
...
export const startNetworkCaptureTool = tool({
  ...
  description:
    ...
    "Only one capture can run per conversation at a time — call stop_network_capture (or get_network_capture_status) if one may already be running. Bounded to the most recent 2000 requests; if the reproduction is that noisy, prefer a narrower time window and get_network_diagnostics instead.",
  parameters: z.object({}),
...
export const stopNetworkCaptureTool = tool({
  ...
  execute: async ({ onlyErrors }, context) => {
    const conversationId = (context as ToolRunContext)?.context?.conversationId;
    recordToolCall(conversationId, "stop_network_capture", { onlyErrors });
...
export const getNetworkCaptureStatusTool = tool({
  ...
  execute: async (_input, context) => {
    const conversationId = (context as ToolRunContext)?.context?.conversationId;
    recordToolCall(conversationId, "get_network_capture_status");
