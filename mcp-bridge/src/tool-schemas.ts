...
When sendToLLM=true: Sends image to LLM (higher latency/cost) and enables coordinate-based actions.`,
  ...
  // ===== Selector Diagnostics Tool =====
  {
    ...
    description:
      "Analyze an element (found via search_elements's uid) and recommend a stable CSS/XPath selector for it. Generates candidate selectors, flags dynamic-looking values, and tests every candidate against the live page. Only works for CDP-mode snapshots.",
    inputSchema: {
      type: "object",
      properties: {
        tabId: {
        ...
        uid: {
          type: "string",
          description:
            "The unique identifier of an element from the page snapshot (from search_elements)",
        },
      },
      required: ["tabId", "uid"],
    },
  ...
  // ===== Investigation Tools =====
  {
    ...
    description:
      "Start a first-class investigation for this conversation: records the reported problem, generates a deterministic investigation plan, and puts the investigation into an explicit lifecycle. Call once, early, when actually investigating a reported Apty/browser issue.",
    inputSchema: {
      type: "object",
      properties: {
        userProblem: {
          type: "string",
          description: "The problem reported, verbatim or a faithful summary",
        },
        suspectedComponents: {
          type: "array",
          items: {
          ...
          description: "The Apty component(s) already suspected, if apparent",
        },
      },
  ...
  },
  {
    ...
    description:
      "Get the current investigation's plan — the ordered checklist of steps (with suggested tools) generated by start_investigation, and which are pending vs. done/skipped.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
  {
    name: "update_investigation",
    description:
      "Update the current investigation's lifecycle state: status, hypotheses (add/update), suspected component, diagnosis+confidence, and plan step progress. Only pass the fields being changed. Requires an investigation already started.",
    inputSchema: {
      type: "object",
      properties: {
        status: {
        ...
        addHypothesis: { type: "string" },
        updateHypothesis: {
          type: "object",
          description:
            "{id, status?, confidence?, addSupportingEvidenceId?, addContradictingEvidenceId?}",
        },
        ...
        completedPlanStepId: { type: "string" },
        skippedPlanStepId: { type: "string" },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "Record the outcome of an attempt to verify a hypothesis or the current diagnosis — 'confirmed' when the re-check supports it, 'not_confirmed' when it contradicts it, 'inconclusive' when unclear. Required before update_investigation's confidence can be set to 'confirmed'.",
    inputSchema: {
      type: "object",
      properties: {
        outcome: {
  ...
  },
  {
    ...
    description:
      "Stop the current investigation ('resolved' once explained/fixed, 'failed' if no cause found, 'stopped' if cancelled). Collected evidence is not discarded.",
    inputSchema: {
      type: "object",
      properties: {
        status: { type: "string", enum: ["stopped", "resolved", "failed"] },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "Get the current investigation's lifecycle state — status, problem, suspected components, hypotheses, diagnosis/confidence, verification attempts, and evidence count.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
  {
    name: "get_investigation_timeline",
    description:
      "Get a deterministically correlated timeline of all diagnostic evidence collected so far in this conversation, clustered by shared request/correlation id or time-window proximity.",
    inputSchema: {
      type: "object",
      properties: {
        windowMs: {
          type: "number",
          description:
            "Max gap in ms between two events to cluster them without a shared id (default 2000)",
        },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "Clear all diagnostic evidence collected so far in this conversation, discarding the correlated timeline. Use when starting a fresh, unrelated investigation in the same chat.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
  {
