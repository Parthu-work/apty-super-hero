...
When sendToLLM=true: Sends image to LLM (higher latency/cost) and enables coordinate-based actions.`,
  ...
  // ===== Investigation-Aware Network Capture Tools =====
  {
    ...
    description:
      "Start capturing network requests on the current tab in the background — unlike get_network_diagnostics, this does not block on a fixed time window. Call this right before reproducing the issue, then call stop_network_capture whenever ready to see what happened. Tagged with the active investigation, if any. Only one capture can run per conversation at a time.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
  {
    name: "stop_network_capture",
    description:
      "Stop the network capture started by start_network_capture and return every request seen while it was running — correlated request/response pairs, status codes, resource types, and failures. Failed/4xx/5xx requests are also recorded as investigation evidence. Returns an error if no capture is currently running.",
    inputSchema: {
      type: "object",
      properties: {
        onlyErrors: {
          type: "boolean",
          description:
            "If true, only return failed requests or responses with status >= 400. Default false.",
        },
      },
      required: [],
    },
  ...
  // ===== Selector Diagnostics Tools =====
  {
    name: "analyze_element_selectors",
    description:
      "Analyze an element (found via search_elements/take_snapshot's uid) and recommend a stable CSS/XPath selector for it — Apty's core 'why can't Studio select this element' question. Generates ranked candidate selectors, flags dynamic-looking values, and tests every candidate against the live page. Only works for elements found via a CDP-mode snapshot.",
    inputSchema: {
      type: "object",
      properties: {
        tabId: {
          type: "number",
          description: "The ID of the tab the element is on",
        },
        uid: {
          type: "string",
          description:
  ...
  // ===== Investigation Lifecycle Tools =====
  {
    ...
    description:
      "Get a deterministically correlated timeline of all diagnostic evidence collected so far in this conversation, clustered by shared request/correlation id or time-window proximity. Call diagnostic tools first — this only correlates what's already been collected.",
    inputSchema: {
      type: "object",
      properties: {
        windowMs: {
          type: "number",
          description:
            "Max gap in ms between two events for them to be grouped into the same cluster (100-30000, default 2000).",
        },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "Clear all diagnostic evidence collected so far in this conversation, discarding the correlated timeline. Use when starting a fresh investigation in the same chat.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
  {
    name: "start_investigation",
    description:
      "Start a first-class investigation for this conversation: records the user's reported problem and suspected Apty component(s), generates a deterministic investigation plan, and puts the investigation into an explicit lifecycle. Calling it again replaces the previous investigation.",
    inputSchema: {
      type: "object",
      properties: {
        userProblem: {
          type: "string",
          description:
            "The problem the user reported, in their own words or a faithful summary.",
        },
        suspectedComponents: {
          type: "array",
          description:
            "The Apty component(s) already suspected: 'apty-client-widget-player' (the runtime extension) and/or 'apty-studio'.",
          items: {
            type: "string",
            enum: ["apty-client-widget-player", "apty-studio"],
          },
        },
      },
      required: ["userProblem"],
    },
  },
  {
    name: "get_investigation_plan",
    description:
      "Get the current investigation's plan — the ordered checklist of investigation steps generated by start_investigation, and which are still pending vs. done/skipped. Advisory, not mandatory.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
  {
    name: "update_investigation",
    description:
      "Update the current investigation's lifecycle state — status, hypotheses, suspected components, diagnosis/confidence, and plan step progress. Only pass the fields being changed. Confidence 'confirmed' is only accepted if a confirmed verification attempt has already been recorded, otherwise it's downgraded to 'likely'. Requires an investigation already started.",
    inputSchema: {
      type: "object",
      properties: {
        status: {
        ...
        addHypothesis: {
          type: "string",
          description:
            "Record a new candidate explanation (starts open/unknown confidence).",
        },
        updateHypothesis: {
          type: "object",
          description:
            "Move an existing hypothesis forward, citing evidence ids.",
          properties: {
            id: { type: "string" },
            status: {
              type: "string",
            ...
            addSupportingEvidenceId: { type: "string" },
            addContradictingEvidenceId: { type: "string" },
          },
        },
        addSuspectedComponent: {
          type: "string",
          enum: ["apty-client-widget-player", "apty-studio"],
        },
        ...
        completedPlanStepId: {
          type: "string",
          description: "Mark a step from get_investigation_plan as done.",
        },
        skippedPlanStepId: {
          type: "string",
          description:
  ...
  },
  {
    ...
    description:
      "Record the outcome of an attempt to verify a hypothesis or the current diagnosis via a real, deterministic re-check. A 'confirmed' outcome is required before update_investigation's diagnosis confidence can actually be set to 'confirmed'. Requires an investigation already started.",
    inputSchema: {
      type: "object",
      properties: {
        outcome: {
          type: "string",
          enum: ["confirmed", "not_confirmed", "inconclusive"],
        },
        notes: {
          type: "string",
          description: "What the re-check actually found, briefly.",
        },
        hypothesisId: {
          type: "string",
          description: "The hypothesis this attempt was testing, if any.",
        },
      },
      required: ["outcome"],
    },
  },
  {
    ...
    description:
      "Stop the current investigation. Use 'resolved' once explained/fixed, 'failed' when no cause could be determined, or 'stopped' (default) when cancelled. Collected evidence is not discarded.",
    inputSchema: {
      type: "object",
      properties: {
        status: {
          type: "string",
          enum: ["stopped", "resolved", "failed"],
          description: "Default 'stopped'.",
        },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "Get the current investigation's lifecycle state — status, reported problem, suspected components, hypotheses, diagnosis/confidence, and verification attempts, plus how much evidence has been collected.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
